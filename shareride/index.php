<?php
	...
	if(mysqli_query($conn, $sql))
	{
		...
		$sql2 = "SELECT * FROM majorproject`.`Rides` WHERE `DriverName` = '$name' AND `DriverEmail` = '$email' AND `DriverMobile` = '$mobile' AND `From` = '$from' AND `To` = '$to' AND `Date` = '$date' AND `Time` = '$time' AND `CarNo` = '$carno' AND `CarName` = '$carname' AND `NoOfSeats` = '$noofseats' AND `Price` = '$price';";
		...
		$sql3 = "INSERT INTO `majorproject`.`JoinedRides` (`JoinedRideID`, `RideID`, `PassengerName`, `PassengerEmail`, `PassengerMobile`, `NoOfSeatsBooked`, `DriverName`, `DriverEmail`, `DriverMobile`, `From`, `To`, `Date`, `Time`, `CarNo`, `CarName`, `NoOfSeats`, `Price`) VALUES (NULL, '$ride_id', '$name', '$email', '$mobile', NULL, '$name', '$email', '$mobile', '$from', '$to', '$date', '$time', '$carno', '$carname', '$noofseats', '$price');";
